...
**Versi:** 9.1 | **Diperbarui:** 30 Mei 2026 | **Status:** Living Document — Satu-satunya sumber kebenaran  
...
## RINGKASAN EKSEKUTIF (per 30 Mei 2026)
...
| Route kurir (`/kurir/*`) | **6** (termasuk `/kurir/withdraw`) |
| Route publik / marketplace | **~62** |
| Tabel Supabase (public schema) | **151** |
| Tabel Drizzle/Neon (payment_transactions, webhook_logs) | **2** |

**Status keseluruhan (v9.1):** Fase 1–10 selesai 100% dari sisi kode dan route. Sisa pekerjaan: **2 blocker kritis** (payment secrets + email secrets — harus diset user di Replit Secrets) dan **3 langkah DB** (apply schema Supabase, regenerasi types.ts, verify rename tabel). Seluruh kode infrastruktur, routing, dan fitur sudah tersedia.

---

## 🚦 STATUS TERKINI — APA YANG SUDAH DAN BELUM SELESAI (30 Mei 2026)

### ✅ SUDAH SELESAI (Kode & Infrastruktur)

| Area | Detail |
|---|---|
| **303 route frontend** | Semua route tersedia: 61 admin, 154 pos-app, 21 akun, 6 kurir, 62+ publik |
| **JWT middleware PostgREST** | Decode JWT + table whitelist + shop_id scoping di `rest.ts` (F1-1) |
| **Rate limiting & CORS** | `express-rate-limit` terpasang, CORS restricted ke Replit domain (F1-4) |
| **Payment kode** | `payments.ts`, `midtrans.js`, `xendit.js`, `gateway-config.js` lengkap (F1-2) |
| **Email kode** | `email.ts` dengan Resend, template undangan + renewal tersedia (F1-3) |
| **SSE Realtime** | Broker SSE (POS, kurir, notifikasi) + tracking publik live (Fase 5) |
| **Group Buy, Langganan, BNPL, Telemedicine, Live Commerce** | Route + SQL migration (Fase 6) |
| **Lokasi & Peta** | GPS, Nominatim, Leaflet, clustering, `/sekitar`, `/toko/$slug/map` (Fase 7) |
| **Fitur Kurir lengkap** | Upload foto, penarikan, peta navigasi, rating, riwayat filter (F10-1–F10-5) |
| **Staff/Kasir** | Dashboard shift, slip gaji digital (F10-6–F10-7) |
| **Admin tools** | Commission, auto-cancel, GDPR, broadcast-buyers, churn, webhook-monitor, API keys (Fase 9) |
| **Quick Wins** | QW1–QW9 semua selesai (skeleton, empty state, SEO, OG tags, dll.) |
| **Auth proxy JSON error** | Fixed: `.json()` pada response kosong (204/304) tidak crash lagi |
| **Paket npm** | `resend`, `express-rate-limit`, `pg` terinstall di `api-server` |
| **SQL scripts idempotent** | `scripts/fresh_schema/*.sql` aman dijalankan berulang kali (IF NOT EXISTS, DO NOTHING) |

### ❌ BELUM SELESAI — PERLU ACTION USER / DEPLOYMENT

#### 🔴 KRITIS — Harus dilakukan sebelum go-live

| ID | Item | Tindakan |
|----|------|----------|
| **BL-1** | `MIDTRANS_SERVER_KEY` belum dikonfigurasi | Set di **Replit Secrets** → checkout marketplace gagal bayar |
| **BL-2** | `XENDIT_SECRET_KEY` belum dikonfigurasi | Set di **Replit Secrets** → checkout Xendit gagal |
| **BL-3** | `RESEND_API_KEY` belum dikonfigurasi | Set di **Replit Secrets** → undangan staff & renewal tidak terkirim |
| **BL-4** | Schema Supabase belum di-apply | Jalankan `scripts/fresh_schema/run_fresh_schema.sh` di Supabase SQL Editor |
| **BL-5** | `types.ts` belum diregenerasi | Jalankan `npx supabase gen types typescript --project-id <ID>` setelah schema di-apply |

#### 🟡 PENTING — Perlu dilakukan untuk fitur lengkap

| ID | Item | Tindakan |
|----|------|----------|
| **BL-6** | `coffee_shops → shops` belum diverify di DB Supabase aktual | Cek + jalankan `ALTER TABLE IF EXISTS public.coffee_shops RENAME TO shops` |
| **BL-7** | `shops_nearby` RPC belum diverify ada di Supabase | Jalankan SQL dari §10.3 di Supabase SQL Editor |
| **BL-8** | Push Notification VAPID keys belum dikonfigurasi | Set `VAPID_PUBLIC_KEY` + `VAPID_PRIVATE_KEY` di Replit Secrets |
| **BL-9** | `RAJAONGKIR_API_KEY` belum dikonfigurasi | Set di Replit Secrets → halaman `/pos-app/rajaongkir` tidak bisa fetch ongkir aktual |
| **BL-10** | JWT signature verification decode-only | `rest.ts` decode JWT tapi tidak verify signature — `SUPABASE_JWT_SECRET` perlu diset untuk full verification |

#### 🟢 RENDAH — Nice to have

| ID | Item | Keterangan |
|----|------|------------|
| **BL-11** | Broadcast WA aktual | UI + wa.me deep-link sudah ada; integrasi full WhatsApp Business API (360dialog/Twilio) perlu credentials merchant |
| **BL-12** | pg_cron untuk auto-cancel & churn | `fn_auto_cancel_expired` dan `fn_churn_metrics_snapshot` sudah ada; perlu enable pg_cron di Supabase |
| **BL-13** | `menu_item_options → product_options` rename | Opsional, rendah-prioritas, 6 file terdampak |
| **BL-14** | Mobile App (React Native/Expo) | Di luar scope proyek ini — proyek terpisah |
...
### K1 — Keamanan PostgREST Proxy ✅ SELESAI (parsial)

**Status:** JWT decode + table whitelist + `shop_id` scoping sudah ada di `rest.ts`.  
**Sisa:** JWT signature verification masih decode-only — `SUPABASE_JWT_SECRET` perlu diset di Replit Secrets untuk full verification (BL-10). Fungsionalitas tetap berjalan, tapi token palsu bisa lolos tanpa secret.

### K2 — File Upload / Storage ✅ ACCEPTABLE

**Status:** Supabase Storage dipakai untuk semua upload — bucket policy dikonfigurasi via `scripts/fresh_schema/04_policies_and_storage.sql`. Ini acceptable untuk fase sekarang.  
**Jika ingin migrasi:** Opsi Cloudinary / Cloudflare R2 tersedia tapi bukan prioritas saat ini.

### K3 — Realtime / WebSocket ✅ SELESAI

**Status:** SSE broker (`/api/sse/stream`) menggantikan Supabase Realtime untuk channel kritis: POS (`channel=pos`), kurir (`channel=courier`), notifikasi merchant (`channel=notifications`). Tracking publik live via `/track/:orderId`.

### K4 — Payment Gateway Secrets ❌ BELUM — BLOCKER GO-LIVE

**Status:** Kode 100% siap (`payments.ts`, `midtrans.js`, `xendit.js`). Belum bisa go-live karena:
- `MIDTRANS_SERVER_KEY` → belum di-set di Replit Secrets (BL-1)
- `XENDIT_SECRET_KEY` → belum di-set di Replit Secrets (BL-2)

**Tindakan:** Set kedua secrets di Replit Secrets panel → restart API Server → uji checkout end-to-end.

### K5 — Email Delivery ❌ BELUM — BLOCKER GO-LIVE

**Status:** Kode 100% siap (`email.ts` dengan Resend, template undangan + renewal). Belum bisa mengirim email karena:
- `RESEND_API_KEY` → belum di-set di Replit Secrets (BL-3)
- Saat ini: jika key tidak ada, email di-log ke console saja (dev mode, tidak crash)

**Tindakan:** Daftar di [resend.com](https://resend.com) → Set `RESEND_API_KEY` di Replit Secrets → Set `EMAIL_FROM` ke domain terverifikasi.
...
| # | Masalah | Status |
|---|---|---|
| T1 | PostgREST proxy tanpa auth | ✅ JWT decode + table whitelist + shop_id scoping (F1-1) — sisa: set `SUPABASE_JWT_SECRET` untuk full signature verify (BL-10) |
| T2 | SQL Injection via `table` param | ✅ Table whitelist di `rest.ts` — hanya tabel yang di-approve bisa diakses |
| T3 | Row Level Security di proxy | ✅ Auto-inject `shop_id` filter dari JWT claims |
| T4 | CORS terlalu terbuka | ✅ CORS restricted ke Replit domain + origin whitelist via `ALLOWED_ORIGINS` env (F1-4) |
| T5 | Tidak ada rate limiting | ✅ `express-rate-limit` terpasang: 200 req/menit global, 20 req/menit untuk auth+payment (F1-4) |

### 5.2 Data Mock / Fake — Status per 30 Mei 2026

| File Route | Status |
|---|---|
| `pos-app.email-marketing.tsx` | ✅ Done — migrasi ke `marketing_campaigns` + `campaign_recipients` (F2-1) |
| `pos-app.storefront-builder.tsx` | ✅ Done — wire save/load ke `storefront_layouts` di Supabase (F2-2, F8-4) |
| `pos-app.digital-version.tsx` | ✅ Done — wire ke `menu_items` + `digital_product_versions` (F2-3) |
| `pos-app.rajaongkir.tsx` | ✅ Done — Express endpoint `/api/rajaongkir/cost` + `/api/rajaongkir/city` (F2-4) — `RAJAONGKIR_API_KEY` masih pending (BL-9) |
| `pos-app.flash-sale.tsx` + `pos-app.happy-hour.tsx` | ✅ Done — kolom `flash_price`, `flash_starts_at`, `flash_ends_at`, `happy_hour_*` di migration SQL (F2-5, F8-5, F8-6) |
| `pos-app.bulk-pricing.tsx` | ✅ Done — tabel `bulk_pricing_rules` di `scripts/fase8_fase9_migrations.sql` (F8-2) |
| `pos-app.restock-notify.tsx` | ✅ Done — tabel `restock_subscribers` + fn `fn_notify_restock` + endpoint `POST /admin/restock-notify` (F8-3) |
| `pos-app.broadcast-wa.tsx` | ⚠️ Parsial — UI + wa.me deep-link tersedia; integrasi full WhatsApp Business API perlu credentials merchant (BL-11) |

### 5.3 Duplikasi Kode — Status

| Area | Status |
|---|---|
| **Auth routes** (`masuk.tsx`, `daftar.tsx`) | ✅ Done — jadi redirect shim ke `login.tsx` + `signup.tsx` (F3-1) |
| **Cart system** (3 sistem berbeda) | ✅ Diterima — `lib/cart.ts` (POS), `lib/customer-cart.ts` (per-toko), `lib/marketplace-cart.ts` (multi-toko); sudah terdokumentasi |
| **Printer** (`escpos-printer.ts` legacy) | ✅ Done — `escpos-printer.ts` jadi re-export shim ke `lib/printer/escpos-compat.ts` (F3-2) |
| **Review dialog** | ✅ Done — komponen terpadu di `components/shared/ReviewDialog.tsx`; file lama jadi wrapper tipis (F3-3) |
| **Shop context** | ✅ Done — `lib/shop-context.tsx` menggabungkan `use-shop.ts` + `outlet-context.tsx`; file lama jadi re-export shim (F3-4) |

### 5.4 Performa & Realtime

| Masalah | Status |
|---|---|
| Subscribe realtime tanpa filter `shop_id` | ✅ Done — semua channel owner sudah punya filter `shop_id` (F3-5) |
| List besar tanpa cursor pagination | ✅ Done — keyset pagination di `pos-app.customers.tsx` + indeks DB (F3-6) |
| View legacy `coffee_shops` | ⚠️ Perlu verifikasi di DB Supabase aktual apakah sudah di-rename (BL-6) |

### 5.5 Operator PostgREST Proxy

| Operator | Status |
|---|---|
| `.in`, `.not`, `.or` | ✅ Done — diimplementasi di `rest.ts` (F1-5) |
| Embedded resources `select=*,shop:shops(name)` | ⚠️ JOIN resolver belum ada — low priority |
| `RETURNING *` setelah POST/PATCH | ✅ Done — response konsisten (F1-5) |
...
## BAGIAN 8 — METRIK KELENGKAPAN PER ROLE (per 30 Mei 2026)

| Role | Route Ada | Status Kode | Blocker Tersisa | % Kode Siap |
|---|---|---|---|---|
| **Super Admin** | 61 route ✅ | Semua route lengkap + DB wired | Jalankan SQL migration di Supabase (BL-4) | **~97%** |
| **Merchant / Owner** | 154 route ✅ | Semua route + DB connected | Broadcast WA perlu credentials (BL-11) | **~98%** |
| **Staff / Kasir** | (via pos-app) ✅ | Dashboard shift ✅, slip gaji ✅ | `RESEND_API_KEY` untuk undangan email (BL-3) | **~95%** |
| **Customer / Buyer** | 21 route ✅ | Semua route tersedia | `MIDTRANS/XENDIT` secrets untuk checkout (BL-1, BL-2) | **~95%** |
| **Kurir** | 6 route ✅ | Upload foto ✅, penarikan ✅, peta ✅, rating ✅, riwayat filter ✅ | — | **~100%** |
| **Marketplace Publik** | 62+ route ✅ | Search ✅, lokasi ✅, group buy ✅, live ✅, telemedicine ✅ | — | **~100%** |
| **Infrastruktur** | — | JWT ✅, rate limit ✅, SSE ✅, payment kode ✅, email kode ✅ | Set 3 Replit Secrets (BL-1–3) + apply DB schema (BL-4) | **Kode 95%, Deploy 60%** |

> **Interpretasi "%"**: Angka mencerminkan *kelengkapan kode*, bukan kesiapan produksi. Platform siap go-live secara penuh setelah BL-1 s/d BL-5 diselesaikan.

**Progres Roadmap (per 30 Mei 2026):**

| Fase | Nama | Status |
|---|---|---|
| Fase 1 | Infrastruktur Wajib | ✅ **Selesai** — kode 100%; BL-1, BL-2, BL-3 pending secrets |
| Fase 2 | Sambungkan Mock Data | ✅ **Selesai** |
| Fase 3 | Bersih-bersih Kode | ✅ **Selesai** |
| Fase 4 | Customer Experience | ✅ **Selesai** |
| Fase 5 | Realtime SSE | ✅ **Selesai** |
| Fase 6 | Platform Lanjutan | ✅ **Selesai** (F6-7 di-skip — proyek terpisah) |
| Quick Wins | QW1–QW9 | ✅ **9/9 Selesai** |
| Lokasi P1+P2 | L1-1 s/d L2-5 | ✅ **9/9 Selesai** |
| Fase 7 | DB Migration + Lokasi P3 | ✅ **Selesai** — SQL siap; L3-1 s/d L3-5 ✅ |
| Fase 8 | Merchant Mock Lanjutan | ✅ **6/6 Selesai** |
| Fase 9 | Admin Platform Tools | ✅ **8/8 Selesai** |
| Fase 10 | Kurir & Staff Lengkap | ✅ **7/7 Selesai** |
| **Go-Live** | Secrets + DB apply | ❌ **Pending** — lihat BL-1 s/d BL-5 di §STATUS TERKINI |

> **Kesimpulan (30 Mei 2026):** Seluruh fase kode selesai. **Satu-satunya yang menghalangi go-live adalah: set 3 Replit Secrets (MIDTRANS_SERVER_KEY, XENDIT_SECRET_KEY, RESEND_API_KEY) + jalankan SQL schema di Supabase + regenerasi types.ts.** Semua SQL migration tersedia di `scripts/`. Detail tindakan di bagian 🚦 STATUS TERKINI di atas.
...
*Versi 9.0 (29 Mei 2026): Analisis langsung terhadap 151 tabel Supabase, 2 tabel Drizzle/Neon, 303 route frontend, 8 API route. Fase 1–10 selesai dari sisi kode.*

*Versi 9.1 (30 Mei 2026): Audit ulang codebase — verifikasi implementasi aktual vs status PRD. Perbaikan sesi: auth proxy JSON error (204/304), paket npm resend+express-rate-limit+pg, SQL scripts idempotent (IF NOT EXISTS / DO NOTHING). Update status Bagian 4 (Masalah Kritis), Bagian 5 (Teknis), Bagian 8 (Metrik). Tambah §STATUS TERKINI dengan daftar ✅/❌ yang akurat dan 14 blocker teridentifikasi (BL-1 s/d BL-14). Kode 100% siap; go-live memerlukan BL-1 s/d BL-5 (3 secrets + 2 langkah DB).*
