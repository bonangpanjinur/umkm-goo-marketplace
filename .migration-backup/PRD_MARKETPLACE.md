...
## 20. Feature Gap Matrix (Lintas Role & Terintegrasi)

> Bab ini fokus pada **fitur lintas role yang harus ada agar marketplace terasa hidup**, dipisah dari roadmap backend. Status: ✅ ada · 🟡 sebagian (UI/sebagian alur ada, belum nyambung penuh) · 🔴 belum.
...
| Live order tracking + ETA + map sederhana | 🔴 | lihat status realtime | update status di POS/KDS | SLA dashboard | hook eskalasi dispute |
| In-app chat per order (text + foto) | 🔴 | tanya kurir/toko | balas dari panel | moderasi pesan | read-only saat dispute |
| Dispute & refund center | 🟡 | ajukan dengan bukti | tandai bukti operasional | proses refund | putusan akhir + escrow hold |
...
| Auto-cancel & reminder bayar | 🔴 | countdown bayar | — | aturan per toko | aturan global default |
...
| Voucher platform vs voucher toko (stacking rules) | 🟡 | UI ada, aturan stacking belum jelas |
...
| Produk digital (file/lisensi auto-deliver) | 🔴 | sudah disebut di §9.4 |
...
| Kalender promo + scheduler flash sale | 🔴 | tampilan kalender |
...
| Fitur | Status | Catatan |
|---|---|---|
| Multi kurir + tarif realtime + label print | 🟡 | API kurir belum integrasi |
...
| Fitur | Status | Catatan |
|---|---|---|
| Role granular per modul (lihat memory `staff-permissions`) | ✅ | perluas ke modul marketplace |
| Aktivitas staff per shift (timeline) | 🟡 | shifts ada, timeline belum |
| Audit log dengan filter & export | 🟡 | UI minim |
| Impersonation aman utk Super Admin (banner + audit) | 🔴 | support tooling |
...
| Fitur | Status | Catatan |
|---|---|---|
| Dashboard kesehatan platform (GMV, take rate, churn, SLA) | 🟡 | analytics ada, KPI belum lengkap |
...
Tulang punggung pengalaman terintegrasi (≥3 role tersentuh tiap item):

1. **Order timeline tunggal** — pembeli, kasir, kurir, owner, admin lihat event yang sama.
2. **Inbox terpadu** — chat, notifikasi sistem, komentar review masuk satu bell di setiap role.
3. **Saldo & payout transparan** — pembeli lihat status refund = owner lihat escrow = admin lihat ledger.
4. **Stok single source of truth** — kasir POS & pembeli marketplace tak pernah lihat stok beda.
5. **Identitas & verifikasi** — KYC owner muncul sebagai badge di toko publik & filter di admin.

---

## 21. UI/UX Blueprint per Role (Responsif Penuh)

> Prinsip: **setiap layout wajib jalan mulus di mobile, tablet, desktop**. Tiap role tetap punya **device utama** yang dioptimalkan ekstra (POS = tablet, pembeli = mobile, super admin = desktop), tapi tidak boleh ada layar yang patah di device lain.
...
- **Konsistensi**: 1 sistem ikon (lucide), 1 toast (sonner), 1 pola tabel (shadcn `Table` + virtualization saat >100 row).

### 21.2 Pembeli (Customer) — Mobile-first, mulus di tablet/desktop
...
[ feed produk     ]   [ side tab navigasi  ] [ sticky cart drawer kanan ]
[ bottom nav 5 ikon ]
```

- **Bottom nav 5 ikon** mobile: Home, Cari, Keranjang, Pesanan, Akun.
- **Sticky CTA "Beli Sekarang"** di detail produk (mobile) → fixed bottom bar.
- **Checkout 1 layar** (alamat, kurir, bayar) accordion; desktop 2 kolom (form | summary).
...
### 21.3 Pemilik Toko (Owner) — Desktop & Tablet
...
- **Sidebar grup collapsible** — di mobile jadi `Sheet` dari kiri; di tablet auto-collapse jadi ikon-only.
- **Pesanan Hub** unified: tab `Semua · POS · Web · Marketplace` + filter chip status, kurir, channel.
- **Quick actions strip** top bar: tambah produk, broadcast, lihat saldo, scan QR.
- **Form berat** (produk, promo) → 2 kolom desktop (form | live preview), stepper di mobile.
- **Tabel besar** → mobile jadi card list dgn aksi swipe.
...
- **Touch-first**: tombol min 56px, font ≥16px, tanpa state hover-only.
- **Keyboard shortcut** di desktop (F2 cari, F3 diskon, Enter charge).
- **KDS** terpisah: kartu order auto-flow, warna status, sound saat order baru.
...
- **Data dense table**: column toggle, saved view, bulk action bar saat ada selection, export CSV.
- **KYC review** = split: dokumen kiri zoomable, form keputusan kanan, riwayat di bawah.
- **Broadcast composer** dengan preview multi-channel berdampingan.
- **Realtime alerts**: toast utk hal baru + drawer log persistent.

### 21.6 Komponen Bersama Baru (semua responsif)

- `OrdersTabs` — diperluas dgn badge counter realtime.
- `NotificationBell` + `NotificationDrawer` (channel terfilter per role).
- `ChatPanel` (bottom sheet mobile / side drawer desktop).
- `StatusTimeline` (vertikal mobile, horizontal desktop).
- `SaldoCard` 3 segmen warna (pending/available/withdrawn).
