-- 1) Extend payment_method enum to cover marketplace options
ALTER TYPE public.payment_method ADD VALUE IF NOT EXISTS 'manual_transfer';
ALTER TYPE public.payment_method ADD VALUE IF NOT EXISTS 'cod';

-- 2) Replace marketplace_checkout: accept _payment_method, normalize it, and use it in the INSERT
DROP FUNCTION IF EXISTS public.marketplace_checkout(text, text, text, text, text, jsonb, jsonb, text);
...
CREATE OR REPLACE FUNCTION public.marketplace_checkout(
  ...
  _payment_method text DEFAULT 'manual_transfer'::text,
...
GRANT EXECUTE ON FUNCTION public.marketplace_checkout(text, text, text, text, text, text, jsonb, jsonb, text) TO authenticated;