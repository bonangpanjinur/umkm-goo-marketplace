...
import {
  ...
  CreditCard, Mail, Bell, MessageCircle, Info,
} from "lucide-react";
...
function StatusBadge({ configured }: { configured: boolean }) {
  ...
  return (
    <Badge variant="outline" className="text-muted-foreground gap-1">
      <XCircle className="h-3 w-3" /> Belum diatur
    </Badge>
  );
}
...
function AdminCredentials() {
  ...
  const invalidateServerCache = async () => {
    const adminSecret = prompt("Masukkan ADMIN_SECRET untuk refresh cache server:");
    ...
    try {
      ...
      else toast.error("Gagal refresh cache. Pastikan ADMIN_SECRET benar.");
    } catch {
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8 py-6 lg:py-10 space-y-8">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold">Kredensial & API Keys</h1>
          <p className="mt-1 text-sm text-muted-foreground">
            Konfigurasi API keys untuk payment gateway, email, push notification, dan WhatsApp.
      ...
      <div className="rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800 flex gap-2">
        ...
        <div>
          <strong>Catatan:</strong> Setelah menyimpan credentials, klik <strong>Refresh Cache Server</strong> agar API server
          membaca nilai terbaru (cache diperbarui setiap 5 menit secara otomatis).
          Untuk efek langsung, gunakan tombol Refresh Cache Server di atas.
        </div>
      </div>
      ...
      <section>
        ...
        <Card className="p-5 space-y-6">
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                ...
                <SecretInput
                  id="mt-server-key"
                  value={payment.midtrans_server_key}
                  onChange={(v) => setPayment((p) => ({ ...p, midtrans_server_key: v }))}
              ...
              <div>
                ...
                <SecretInput
                  id="mt-client-key"
                  value={payment.midtrans_client_key}
                  onChange={(v) => setPayment((p) => ({ ...p, midtrans_client_key: v }))}
            ...
            <div>
              ...
              <div className="flex gap-2 mt-1">
                {(["sandbox", "production"] as const).map((mode) => (
                  <button
                    key={mode}
                    onClick={() => setPayment((p) => ({ ...p, midtrans_mode: mode }))}
                    className={`px-3 py-1.5 rounded-md text-sm border transition-colors ${
                      payment.midtrans_mode === mode
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                ...
                <SecretInput
                  ...
                  onChange={(v) => setPayment((p) => ({ ...p, xendit_secret_key: v }))}
              ...
              <div>
                ...
                <SecretInput
                  id="xen-webhook"
                  value={payment.xendit_webhook_token}
                  onChange={(v) => setPayment((p) => ({ ...p, xendit_webhook_token: v }))}
            ...
            <div>
              ...
              <div className="flex gap-2 mt-1">
                {(["test", "live"] as const).map((mode) => (
                  <button
                    key={mode}
                    onClick={() => setPayment((p) => ({ ...p, xendit_mode: mode }))}
                    className={`px-3 py-1.5 rounded-md text-sm border transition-colors ${
                      payment.xendit_mode === mode
                        ? "bg-primary text-primary-foreground border-primary"
                        : "border-border hover:bg-accent"
                    }`}
                  >
          ...
          <div className="flex justify-end pt-2">
            <Button
              onClick={() => save("payment_credentials", payment)}
              disabled={saving === "payment_credentials"}
            >
              {saving === "payment_credentials" ? (
        ...
        <Card className="p-5 space-y-3">
          <div className="flex items-center justify-between mb-1">
            <div className="text-sm text-muted-foreground">
              Digunakan untuk: undangan staff, renewal reminder, invoice notifikasi.
            </div>
          ...
          <div>
            ...
            <SecretInput
              ...
              onChange={(v) => setEmail((e) => ({ ...e, resend_api_key: v }))}
              placeholder="re_..."
            />
          ...
          <div className="flex justify-end pt-2">
            <Button
              onClick={() => save("email_credentials", email)}
              disabled={saving === "email_credentials"}
            >
              {saving === "email_credentials" ? (
                <Loader2 className="h-4 w-4 animate-spin mr-1.5" />
              ) : (
                <Save className="h-4 w-4 mr-1.5" />
              )}
      ...
      <section>
        ...
        <Card className="p-5 space-y-3">
          <div className="flex items-center justify-between mb-1">
            <div className="text-sm text-muted-foreground">
              Web Push Notification menggunakan protokol VAPID.
            </div>
          ...
          <div>
            ...
            <Input
              ...
              placeholder="BNcRdreALRFXTkOOUHK..."
          ...
          <div>
            ...
            <SecretInput
              id="vapid-private"
              value={push.vapid_private_key}
              onChange={(v) => setPush((p) => ({ ...p, vapid_private_key: v }))}
          ...
          <div className="rounded-md bg-muted p-3 text-xs text-muted-foreground">
            ...
            {" "}di terminal, atau gunakan{" "}
            <a href="https://vapidkeys.com" target="_blank" rel="noopener noreferrer" className="underline">
              vapidkeys.com
            </a>
          </div>
          <div className="flex justify-end pt-2">
            <Button
              onClick={() => save("push_credentials", push)}
              disabled={saving === "push_credentials"}
            >
              {saving === "push_credentials" ? (
                <Loader2 className="h-4 w-4 animate-spin mr-1.5" />
              ) : (
                <Save className="h-4 w-4 mr-1.5" />
              )}
      ...
      <section>
        ...
        <Card className="p-5 space-y-3">
          <div className="flex items-center justify-between mb-1">
            <div className="text-sm text-muted-foreground">
              Untuk notifikasi WhatsApp ke merchant dan pelanggan.
            </div>
            <StatusBadge configured={Boolean(wa.wa_api_key)} />
          </div>
          <div>
            <Label htmlFor="wa-provider" className="text-xs mb-1">Provider</Label>
            <div className="flex gap-2 mt-1">
              {["fonnte", "wablas", "whacenter"].map((p) => (
                <button
                  key={p}
                  onClick={() => setWa((w) => ({ ...w, wa_api_provider: p }))}
                  className={`px-3 py-1.5 rounded-md text-sm border transition-colors capitalize ${
                    wa.wa_api_provider === p
                      ? "bg-primary text-primary-foreground border-primary"
                      : "border-border hover:bg-accent"
                  }`}
                >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              ...
              <SecretInput
                id="wa-key"
                value={wa.wa_api_key}
                onChange={(v) => setWa((w) => ({ ...w, wa_api_key: v }))}
                placeholder="wa_api_token..."
              />
            </div>
            <div>
              ...
              <Input
                id="wa-phone"
                value={wa.wa_phone}
                onChange={(e) => setWa((w) => ({ ...w, wa_phone: e.target.value }))}
          ...
          <div className="flex justify-end pt-2">
            <Button
              onClick={() => save("wa_credentials", wa)}
              disabled={saving === "wa_credentials"}
            >
              {saving === "wa_credentials" ? (
                <Loader2 className="h-4 w-4 animate-spin mr-1.5" />
              ) : (
                <Save className="h-4 w-4 mr-1.5" />
              )}
