    ...
    (async () => {
      ...
      try {
        const { data: adData } = await (supabase as any).from("ad_requests")
          .select("id, ad_type, target_id, target_name, target_image, position, shops(name)")
          .eq("status", "active")
          .in("position", ["homepage_middle", "hero_carousel"])
          .lt("starts_at", now)
          .gt("ends_at", now);
        if (adData) {
          setSponsoredAds(adData.map((a: any) => ({ ...a, shop_name: a.shops?.name ?? "" })));
        }
      ...
      const [cats, shopsRes, newShopsRes, prodRes, flashRes, shopCount, prodCount, allShopCats] = await Promise.all([
        ...
        supabase.from("shops").select("business_category_id").eq("is_active", true),
      ]);
      ...
      try {
        ...
        if (topIds.length > 0) {
          const { data: bestData } = await supabase.from("menu_items").select("id, shop_id, name, price, image_url, slug, rating_avg, rating_count, stock, total_sold, low_stock_threshold, flash_price, flash_starts_at, flash_ends_at, shop:shops(slug, name, logo_url, kyc_status)").in("id", topIds).eq("is_available", true);
